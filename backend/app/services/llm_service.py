"""LLM 调用服务：OpenAI 兼容协议，供编排器 brain 真实生成分镜脚本。

替代 orchestrator._brain() 的硬编码关键词匹配：用 settings.LLM_API_KEY /
LLM_BASE_URL / LLM_MODEL 调 chat/completions，剥离推理模型的 think 推理块，DOTALL 跨行。
"""
import json
import logging
import re
...
async def chat_completion(system_prompt: str, user_prompt: str, temperature: float = 0.7) -> str:
    ...
    # 用同步 httpx + asyncio.to_thread：sandbox 代理对 async httpx 不稳定，
    # 同步路径已验证可用。to_thread 让它在 async 上下文里不阻塞事件循环。
    import asyncio
